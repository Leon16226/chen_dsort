...
def todo(nn, point, c_box, pools, opt, im0s, lock, matrix_park, histogram):

    thresholds = [20, 3, 3, 20, 0.8]

    # 不同处理策略集合
    strategies = {
        0: IiiParkStrategy(nn, point, c_box[0], pools[0][nn], opt, im0s, thresholds[0], lock, matrix_park, histogram) if c_box[0].size != 0 else 'no',
